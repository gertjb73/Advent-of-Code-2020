    ...
    while continue_processing:
        if line_number < number_of_lines and continue_processing:
            if lines_processed[line_number] >= 1:
                print("Line", line_number + 1, "processed before")
                continue_processing = False
            else:
                lines_processed[line_number] += 1
                line_to_process = code_to_process[line_number].split()
                if line_to_process[key] == "nop":
                    # print("Line", line_number + 1, ": ", line_to_process[key], line_to_process[value], " --> ", end="")
...
# boot_code = read_boot_code_file("Day 08-02 - Handheld Halting - Example input.txt")
boot_code = read_boot_code_file("Day 08-02 - Handheld Halting.txt")
